...
#run in malawi/RCT/midline_dec/analysis
rm(list=ls())
library(nnet)
library(lmtest)
library(car)
...
path <- getwd()
path <- strsplit(path, "/midline_dec/analysis")[[1]]
...
midline_dec <- midline_dec[!(midline_dec$farmer_ID) %in% dups,]

### quick look at attrition

dta$interviewed <- merge(dta,midline_dec[c("farmer_ID","q4a")],by="farmer_ID", all.x=T  )$q4a
...
dta$stock_maize_abs <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","q43gy")],by="farmer_ID", all.x=T  )$q43gy))
dta$stock_gnuts_abs <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","q47gy")],by="farmer_ID", all.x=T  )$q47gy))
dta$stock_soy_abs <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","q52gy")],by="farmer_ID", all.x=T  )$q52gy))

midline_dec$q41[midline_dec$q41 =="n/a"] <- NA
midline_dec$q46[midline_dec$q46 =="n/a"] <- NA
midline_dec$q50[midline_dec$q50 =="n/a"] <- NA
...
set <- c("trans.1..q43a", "trans.2..q43a", "trans.3..q43a", "trans.4..q43a", "trans.5..q43a", "trans.6..q43a", "trans.7..q43a")
midline_dec[set] <- lapply(midline_dec[set], function(x)  as.numeric(as.POSIXct(x, format="%Y-%m-%d"))>=1654034400)
midline_dec$sold_maize <- (rowSums(midline_dec[set],na.rm=T) >0) 
midline_dec$sold_maize[is.na(midline_dec$q41)] <- NA

set <- c("trans1.1..q47a", "trans1.2..q47a","trans1.3..q47a","trans1.4..q47a")
midline_dec[set] <- lapply(midline_dec[set], function(x)  as.numeric(as.POSIXct(x, format="%Y-%m-%d"))>=1654034400)
midline_dec$sold_gnuts <- (rowSums(midline_dec[set],na.rm=T) >0)
midline_dec$sold_gnuts[is.na(midline_dec$q46)] <- NA

set <- c("trans2.1..q52a", "trans2.2..q52a","trans2.3..q52a")
midline_dec[set] <- lapply(midline_dec[set], function(x)  as.numeric(as.POSIXct(x, format="%Y-%m-%d"))>=1654034400)
midline_dec$sold_soy <- (rowSums(midline_dec[set],na.rm=T) >0) 
midline_dec$sold_soy[is.na(midline_dec$q50)] <- NA 

dta$sold_maize <-  merge(dta,midline_dec[c("farmer_ID","sold_maize")],by="farmer_ID", all.x=T  )$sold_maize
dta$sold_gnuts <-  merge(dta,midline_dec[c("farmer_ID","sold_gnuts")],by="farmer_ID", all.x=T  )$sold_gnuts
dta$sold_soy <- merge(dta,midline_dec[c("farmer_ID","sold_soy")],by="farmer_ID", all.x=T  )$sold_soy


set <- c("trans.1..group2.q43c", "trans.2..group2.q43c","trans.3..group2.q43c","trans.4..group2.q43c","trans.5..group2.q43c","trans.6..group2.q43c","trans.7..group2.q43c" )
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x)))
midline_dec[set] <- lapply(midline_dec[set],  function(x) replace(x,is.na(x),0))
midline_dec$sold_maize_kg <- rowSums(midline_dec[set]) 
midline_dec$sold_maize_kg[is.na(midline_dec$q41)] <- NA
dta$sold_maize_kg <- merge(dta,midline_dec[c("farmer_ID","sold_maize_kg")],by="farmer_ID", all.x=T)$sold_maize_kg

set_1 <- c("trans.1..q43a", "trans.2..q43a", "trans.3..q43a", "trans.4..q43a", "trans.5..q43a", "trans.6..q43a", "trans.7..q43a")
midline_dec$sold_maize_kg_2 <- rowSums(midline_dec[set]*midline_dec[set_1], na.rm=T)
midline_dec$sold_maize_kg_2[is.na(midline_dec$q41)] <- NA
dta$sold_maize_kg <- merge(dta,midline_dec[c("farmer_ID","sold_maize_kg_2")],by="farmer_ID", all.x=T)$sold_maize_kg_2

set <- c("trans1.1..group4.q47c", "trans1.2..group4.q47c","trans1.3..group4.q47c","trans1.4..group4.q47c")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x)))
midline_dec[set] <- lapply(midline_dec[set],  function(x) replace(x,is.na(x),0))
midline_dec$sold_gnuts_kg <- rowSums(midline_dec[set]) 
midline_dec$sold_gnuts_kg[is.na(midline_dec$q46)] <- NA
dta$sold_gnuts_kg <- merge(dta,midline_dec[c("farmer_ID","sold_gnuts_kg")],by="farmer_ID", all.x=T)$sold_gnuts_kg

set_1 <- c("trans1.1..q47a", "trans1.2..q47a","trans1.3..q47a","trans1.4..q47a")
midline_dec$sold_gnuts_kg_2 <- rowSums(midline_dec[set]*midline_dec[set_1], na.rm=T)
midline_dec$sold_gnuts_kg_2[is.na(midline_dec$q46)] <- NA
dta$sold_gnuts_kg <- merge(dta,midline_dec[c("farmer_ID","sold_gnuts_kg_2")],by="farmer_ID", all.x=T)$sold_gnuts_kg_2
  
set <- c("trans2.1..group6.q52c", "trans2.2..group6.q52c","trans2.3..group6.q52c")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x)))
midline_dec[set] <- lapply(midline_dec[set],  function(x) replace(x,is.na(x),0))
midline_dec$sold_soy_kg <- rowSums(midline_dec[set]) 
midline_dec$sold_soy_kg[is.na(midline_dec$q50)] <- NA 
dta$sold_soy_kg <- merge(dta,midline_dec[c("farmer_ID","sold_soy_kg")],by="farmer_ID", all.x=T)$sold_soy_kg

set_1 <- c("trans2.1..q52a", "trans2.2..q52a","trans2.3..q52a")
midline_dec$sold_soy_kg_2 <- rowSums(midline_dec[set]*midline_dec[set_1], na.rm=T)
midline_dec$sold_soy_kg_2[is.na(midline_dec$q50)] <- NA 
dta$sold_soy_kg <- merge(dta,midline_dec[c("farmer_ID","sold_soy_kg_2")],by="farmer_ID", all.x=T)$sold_soy_kg_2

set <- c("trans.1..q43d", "trans.2..q43d","trans.3..q43d","trans.4..q43d","trans.5..q43d","trans.6..q43d","trans.7..q43d")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x)))
midline_dec$price_maize <- rowMeans(midline_dec[set],na.rm=T) 
dta$price_maize <- merge(dta,midline_dec[c("farmer_ID","price_maize")],by="farmer_ID", all.x=T)$price_maize

##multiply prices with indicator of sales post treatment 
set_1 <- c("trans.1..q43a", "trans.2..q43a", "trans.3..q43a", "trans.4..q43a", "trans.5..q43a", "trans.6..q43a", "trans.7..q43a")
midline_dec$price_maize_2 <-rowMeans(midline_dec[set]*midline_dec[set_1],na.rm=T)
midline_dec$price_maize_2[midline_dec$price_maize_2==0] <- NA
dta$price_maize <- merge(dta,midline_dec[c("farmer_ID","price_maize_2")],by="farmer_ID", all.x=T)$price_maize_2

set <- c("trans1.1..q47d", "trans1.2..q47d","trans1.3..q47d","trans1.4..q47d")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x)))
midline_dec$price_gnuts <- rowMeans(midline_dec[set],na.rm=T) 
dta$price_gnuts <- merge(dta,midline_dec[c("farmer_ID","price_gnuts")],by="farmer_ID", all.x=T)$price_gnuts

set_1 <- c("trans1.1..q47a", "trans1.2..q47a","trans1.3..q47a","trans1.4..q47a")
midline_dec$price_gnuts_2 <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)
midline_dec$price_gnuts_2[midline_dec$price_gnuts_2==0] <- NA
dta$price_gnuts <- merge(dta,midline_dec[c("farmer_ID","price_gnuts_2")],by="farmer_ID", all.x=T)$price_gnuts_2

set <- c("trans2.1..q52d", "trans2.2..q52d","trans2.3..q52d")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x)))
midline_dec$price_soy <- rowMeans(midline_dec[set],na.rm=T) 
dta$price_soy <- merge(dta,midline_dec[c("farmer_ID","price_soy")],by="farmer_ID", all.x=T)$price_soy

set_1 <- c("trans2.1..q52a", "trans2.2..q52a","trans2.3..q52a")
midline_dec$price_soy_2 <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)
midline_dec$price_soy_2[midline_dec$price_soy_2==0] <- NA
dta$price_soy <- merge(dta,midline_dec[c("farmer_ID","price_soy_2")],by="farmer_ID", all.x=T)$price_soy_2

###secondary outcomes
set <- c("trans.1..q43f", "trans.2..q43f","trans.3..q43f","trans.4..q43f","trans.5..q43f","trans.6..q43f","trans.7..q43f")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x))==5)
set_1 <- c("trans.1..q43a", "trans.2..q43a", "trans.3..q43a", "trans.4..q43a", "trans.5..q43a", "trans.6..q43a", "trans.7..q43a")
midline_dec$joint_maize <- rowMeans(midline_dec[set]*midline_dec[set_1],na.rm=T)==1 
dta$joint_maize <- merge(dta,midline_dec[c("farmer_ID","joint_maize")],by="farmer_ID", all.x=T)$joint_maize

set <- c("trans1.1..q47f", "trans1.2..q47f","trans1.3..q47f","trans1.4..q47f")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x))==5)
set_1 <- c("trans1.1..q47a", "trans1.2..q47a","trans1.3..q47a","trans1.4..q47a")
midline_dec$joint_gnuts <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)==1 
dta$joint_gnuts <- merge(dta,midline_dec[c("farmer_ID","joint_gnuts")],by="farmer_ID", all.x=T)$joint_gnuts

set <- c("trans2.1..q52f", "trans2.2..q52f","trans2.3..q52f")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x))==5)
set_1 <- c("trans2.1..q52a", "trans2.2..q52a","trans2.3..q52a")
midline_dec$joint_soy <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)==1 
dta$joint_soy <- merge(dta,midline_dec[c("farmer_ID","joint_soy")],by="farmer_ID", all.x=T)$joint_soy

##what market sold to?
set <- c("trans.1..q43e", "trans.2..q43e","trans.3..q43e","trans.4..q43e","trans.5..q43e","trans.6..q43e","trans.7..q43e")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x))==2)
set_1 <- c("trans.1..q43a", "trans.2..q43a", "trans.3..q43a", "trans.4..q43a", "trans.5..q43a", "trans.6..q43a", "trans.7..q43a")
midline_dec$market_maize <- rowMeans(midline_dec[set]*midline_dec[set_1],na.rm=T)==1 
dta$market_maize <- merge(dta,midline_dec[c("farmer_ID","market_maize")],by="farmer_ID", all.x=T)$market_maize

set <- c("trans1.1..q47e", "trans1.2..q47e","trans1.3..q47e","trans1.4..q47e")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x))==2)
set_1 <- c("trans1.1..q47a", "trans1.2..q47a","trans1.3..q47a","trans1.4..q47a")
midline_dec$market_gnuts <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)==1 
dta$market_gnuts <- merge(dta,midline_dec[c("farmer_ID","market_gnuts")],by="farmer_ID", all.x=T)$market_gnuts

set <- c("trans2.1..q52e", "trans2.2..q52e","trans2.3..q52e")
midline_dec[set] <- lapply(midline_dec[set],  function(x) as.numeric(as.character(x))==2)
set_1 <- c("trans2.1..q52a", "trans2.2..q52a","trans2.3..q52a")
midline_dec$market_soy <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)==1 
dta$market_soy <- merge(dta,midline_dec[c("farmer_ID","market_soy")],by="farmer_ID", all.x=T)$market_soy

### proceeds used for education
midline_dec_c <- midline_dec
set <- c("trans.1..q43g", "trans.2..q43g","trans.3..q43g","trans.4..q43g","trans.5..q43g","trans.6..q43g","trans.7..q43g")
midline_dec[set] <- lapply(midline_dec_c[set],  function(x) as.numeric(as.character(x))==7)
set_1 <- c("trans.1..q43a", "trans.2..q43a", "trans.3..q43a", "trans.4..q43a", "trans.5..q43a", "trans.6..q43a", "trans.7..q43a")
midline_dec$edu_maize <- rowMeans(midline_dec[set]*midline_dec[set_1],na.rm=T)>0 
dta$edu_maize <- merge(dta,midline_dec[c("farmer_ID","edu_maize")],by="farmer_ID", all.x=T)$edu_maize

set <- c("trans1.1..q47g", "trans1.2..q47g","trans1.3..q47g","trans1.4..q47g")
midline_dec[set] <- lapply(midline_dec_c[set],  function(x) as.numeric(as.character(x))==7)
set_1 <- c("trans1.1..q47a", "trans1.2..q47a","trans1.3..q47a","trans1.4..q47a")
midline_dec$edu_gnuts <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)>0 
dta$edu_gnuts <- merge(dta,midline_dec[c("farmer_ID","edu_gnuts")],by="farmer_ID", all.x=T)$edu_gnuts

set <- c("trans2.1..q52g", "trans2.2..q52g","trans2.3..q52g")
midline_dec[set] <- lapply(midline_dec_c[set],  function(x) as.numeric(as.character(x))==7)
set_1 <- c("trans2.1..q52a", "trans2.2..q52a","trans2.3..q52a")
midline_dec$edu_soy <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)>0 
dta$edu_soy <- merge(dta,midline_dec[c("farmer_ID","edu_soy")],by="farmer_ID", all.x=T)$edu_soy

###expenditure for health
set <- c("trans.1..q43g", "trans.2..q43g","trans.3..q43g","trans.4..q43g","trans.5..q43g","trans.6..q43g","trans.7..q43g")
midline_dec[set] <- lapply(midline_dec_c[set],  function(x) as.numeric(as.character(x))==4)
set_1 <- c("trans.1..q43a", "trans.2..q43a", "trans.3..q43a", "trans.4..q43a", "trans.5..q43a", "trans.6..q43a", "trans.7..q43a")
midline_dec$health_maize <-  rowMeans(midline_dec[set]*midline_dec[set_1],na.rm=T)>0 
dta$health_maize <- merge(dta,midline_dec[c("farmer_ID","health_maize")],by="farmer_ID", all.x=T)$health_maize

set <- c("trans1.1..q47g", "trans1.2..q47g","trans1.3..q47g","trans1.4..q47g")
midline_dec[set] <- lapply(midline_dec_c[set],  function(x) as.numeric(as.character(x))==4)
set_1 <- c("trans1.1..q47a", "trans1.2..q47a","trans1.3..q47a","trans1.4..q47a")
midline_dec$health_gnuts <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)>0 
dta$health_gnuts <- merge(dta,midline_dec[c("farmer_ID","health_gnuts")],by="farmer_ID", all.x=T)$health_gnuts

set <- c("trans2.1..q52g", "trans2.2..q52g","trans2.3..q52g")
midline_dec[set] <- lapply(midline_dec_c[set],  function(x) as.numeric(as.character(x))==4)
set_1 <- c("trans2.1..q52a", "trans2.2..q52a","trans2.3..q52a")
midline_dec$health_soy <- rowMeans(midline_dec[set]*midline_dec[set_1], na.rm=T)>0 
dta$health_soy <- merge(dta,midline_dec[c("farmer_ID","health_soy")],by="farmer_ID", all.x=T)$health_soy

midline_dec$x1[midline_dec$x1=="n/a"] <- NA
midline_dec$x4[midline_dec$x4=="n/a"] <- NA
midline_dec$x7[midline_dec$x7=="n/a"] <- NA

dta$bought_maize <- merge(dta,midline_dec[c("farmer_ID","x1")],by="farmer_ID", all.x=T  )$x1=="Yes"
dta$bought_gnuts <- merge(dta,midline_dec[c("farmer_ID","x4")],by="farmer_ID", all.x=T  )$x4=="Yes"
dta$bought_soy <-merge(dta,midline_dec[c("farmer_ID","x7")],by="farmer_ID", all.x=T  )$x7=="Yes"
...
dta$bought_maize_amt <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","x2")],by="farmer_ID", all.x=T  )$x2))
dta$bought_gnuts_amt <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","x5")],by="farmer_ID", all.x=T  )$x5))
dta$bought_soy_amt <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","x8")],by="farmer_ID", all.x=T  )$x8))
...
dta$bought_maize_price <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","x3")],by="farmer_ID", all.x=T  )$x3))
dta$bought_gnuts_price <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","x6")],by="farmer_ID", all.x=T  )$x6))
dta$bought_soy_price <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","x9")],by="farmer_ID", all.x=T  )$x9))
...
#dta$prod_maize <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","group1.q40")],by="farmer_ID", all.x=T  )$group1.q40.y))
#dta$prod_gnuts <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","group3.q45")],by="farmer_ID", all.x=T  )$group3.q45.y))
#dta$prod_soy <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","group5.q49")],by="farmer_ID", all.x=T  )$group5.q49.y))

#expected prices
dta$price_dec_maize <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","q25")],by="farmer_ID", all.x=T  )$q25.y))
dta$price_dec_soy <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","q28")],by="farmer_ID", all.x=T  )$q28.y))
dta$price_dec_gnuts <- as.numeric(as.character(merge(dta,midline_dec[c("farmer_ID","q31")],by="farmer_ID", all.x=T  )$q31.y))
...
saveRDS(prim_maize, paste(path,"midline_dec/results/prim_maize.RData", sep="/"))
saveRDS(prim_gnuts, paste(path,"midline_dec/results/prim_gnuts.RData", sep="/"))
saveRDS(prim_soy, paste(path,"midline_dec/results/prim_soy.RData", sep="/"))

saveRDS(prim_maize_tc, paste(path,"midline_dec/results/prim_maize_tc.RData", sep="/"))
...
test <- dta[c("farmer_ID","trans.1..q43d", "trans.2..q43d","trans.3..q43d","trans.4..q43d","trans.5..q43d","trans.6..q43d","trans.7..q43d")]
...
test2 <- dta[ c("farmer_ID","trans.1..group2.q43c", "trans.2..group2.q43c","trans.3..group2.q43c","trans.4..group2.q43c","trans.5..group2.q43c","trans.6..group2.q43c","trans.7..group2.q43c" )]
...
maize_sold_2 <- merge(test,test2)
names(maize_sold_2) <-  c("farmer_ID", "trans_ID","price","quant")
maize_sold_2$price[maize_sold_2$price >1500 | maize_sold_2$price <50] <- NA
maize_sold_2$quant[maize_sold_2$quant >100] <- NA

write.csv(maize_sold_2,paste(path,"all_rounds/data/maize_sold_2.csv", sep="/"), row.names = FALSE)
##this is for soy
test <- midline_dec[c("farmer_ID","trans2.1..group6.q52c", "trans2.2..group6.q52c","trans2.3..group6.q52c")]
...
test2 <- midline_dec[c("farmer_ID","trans2.1..q52d", "trans2.2..q52d","trans2.3..q52d")]
...
soy_sold_2 <- merge(test,test2)
names(soy_sold_2) <-  c("farmer_ID", "trans_ID","quant","price")
soy_sold_2$price[soy_sold_2$price >2000 | soy_sold_2$price <50] <- NA
soy_sold_2$quant[soy_sold_2$quant >250] <- NA

write.csv(soy_sold_2,paste(path,"all_rounds/data/soy_sold_2.csv", sep="/"), row.names = FALSE)

#this is for gnuts
test <- midline_dec[c("farmer_ID","trans1.1..group4.q47c", "trans1.2..group4.q47c","trans1.3..group4.q47c","trans1.4..group4.q47c")]
...
test2 <- midline_dec[c("farmer_ID","trans1.1..q47d", "trans1.2..q47d","trans1.3..q47d","trans1.4..q47d")]
...
gnuts_sold_2 <- merge(test,test2)
names(gnuts_sold_2) <-  c("farmer_ID", "trans_ID","quant","price")
gnuts_sold_2$price[gnuts_sold_2$price >15000 | gnuts_sold_2$price <2000] <- NA
gnuts_sold_2$quant[gnuts_sold_2$quant >250] <- NA

write.csv(gnuts_sold_2,paste(path,"all_rounds/data/gnuts_sold_2.csv", sep="/"), row.names = FALSE)
